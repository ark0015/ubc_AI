...
def calDMcurve(data2d, ddms, freqs, period):
    ...
    for i, ddm in enumerate(ddms):
        deltaphases = ddm * 4.15e3 * 1.0 / freqs**2 / period
...
class ar2data(object):
    ...
    def __init__(self, filename, align=True, centre=True):
        ...
        self.freq_lo = self.freq - self.bandwidth / 2.0
        self.freq_hi = self.freq + self.bandwidth / 2.0
        self.freqbins = self.data.shape[1]
        self.binwidth = self.bandwidth / self.freqbins
        self.freqs = np.mgrid[
            self.freq_lo
            + self.binwidth / 2.0 : self.freq_hi
            - self.binwidth / 2.0 : self.freqbins * 1j
        ]
        self.period = archive0.get_folding_period()
        dmfac = 4.15e3 * np.abs(
            1.0 / self.freqs.min() ** 2 - 1.0 / self.freqs.max() ** 2
        )
        ddm = 2.0 * self.period / dmfac
