...
class dataloader(object):
    ...
    def cross_val_score(self, classifier, cv=5, verbose=False):
        ...
        scores = cross_validation(
            classifier, self.pfds, self.target, cv=cv, verbose=verbose
        )
        print("Accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() / 2))
        return scores

    def learning_curve(self, classifier, pct=0.6, plot=True):
        """
    ...
    def feature_curve(
        self, classifier, feature=None, bounds=None, Npts=10, plot=True, pct=0.6
    ):
        """
