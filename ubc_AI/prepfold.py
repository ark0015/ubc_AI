...
class pfd:
    def __init__(self, filename, **kwargs):
        ...
        if swapchar == "<":  # little endian
            ...
            for ii in range(self.npart):
                for jj in range(self.nsub):
                    self.profs[ii, jj, :] = fromfile(infile, np.float64, self.proflen)
        else:
        ...
        for ii in range(self.npart):
            ...
            for jj in range(self.nsub):
                if swapchar == "<":  # little endian
                    currentstats[jj] = fromfile(infile, np.float64, 7)
                else:
        ...
        for k, v in self.__dict__.items():
            if k[:2] != "__":
                if isinstance(self.__dict__[k], str):
                    out += "%10s = '%s'\n" % (k, v)
                elif isinstance(self.__dict__[k], int):
                    out += "%10s = %d\n" % (k, v)
    ...
    def adjust_period(self, p=None, pd=None, pdd=None, interp=0):
        ...
        if interp:
            ...
            new_pdelays_bins = np.floor(bin_delays + 0.5).astype(int)
...
class pfddata(pfd):
    """A wrapper class around prepfold.pfd
    ...
    filename : str,
        the pfd filename
    align : bool, optional
        ensure that binned data falls on max(sum profile).
    ...
    centre : bool, optional
        shift the feature to the phase 0.5
    ...
    def getDMcurve(self, M):
        """returns the normalized DM curve downsampled to M points"""
