...
class cross_validation(object):
    ...
    def cross_val_score(self, classifier, data, target, cv=5, verbose=False):
        ...
        for i in range(cv):
            ...
            if nclasses == 2:
                F1 = singleclass_score(
                    classifier, test_data, test_target, verbose=verbose
                )
            else:
                F1 = multiclass_score(
                    classifier,
...
def multiclass_score(
    classifier, test_data, test_target, nclasses, aclass=None, verbose=True
):
    """
    ...
    for k in range(nclasses):
        ...
        if verbose:
            ...
            print(
                "accuracy: ",
                "%.0f%%" % (np.divide(float(len(hit)), len(true_cls[k])) * 100),
            )
            print(
                "miss: ",
                "%.0f%%" % (np.divide(float(len(miss)), len(true_cls[k])) * 100),
            )
            print(
                "false positives: ",
                "%.0f%%" % (np.divide(float(len(falsepos)), len(pred_cls[k])) * 100),
            )
...
def singleclass_score(classifier, test_data, test_target, verbose=False):
    ...
    if verbose:
        print(
            "accuracy: ", "%.0f%%" % (np.divide(float(len(hit)), len(truepulsar)) * 100)
        )
        print("miss: ", "%.0f%%" % (np.divide(float(len(miss)), len(truepulsar)) * 100))
        print(
            "false positives: ",
            "%.0f%%" % (np.divide(float(len(falsepos)), len(pulsar)) * 100),
        )
        print("precision: ", "%.0f%%" % (precision * 100))
        print("recall: ", "%.0f%%" % (recall * 100))
        print("F1: ", F1)
...
def learning_curve(
    classifier, X, y, Xval=None, yval=None, gamma=None, pct=0.6, plot=False
):
    """
...
def validation_curve(
    classifier, X, y, Xval=None, yval=None, gammas=None, pct=0.6, plot=False
):
    """
...
class datafitter(object):
    ...
    def prepare_data(self, **kwargs):
        ...
        output: [pf.getdata(**kwargs) for pf in self.pfds]
        """
        self.kwargs = kwargs
        self.data = [pf.getdata(**kwargs) for pf in self.pfds]
        self.split()
        # return self.data

    def split(self, pct=0.6):
        train_data, train_target, test_data, test_target = split_data(
            self.data, self.target, pct=pct
        )
    ...
    def cross_val_score(self, classifier, cv=10, verbose=False):
        ...
        scores = cross_validation.cross_val_score(
            classifier, self.data, self.target, cv=cv
        )
    ...
    def feature_curve(
        self, classifier, feature, bounds=None, Npts=10, plot=True, pct=0.4
    ):
        self.kwargs = {}
        self.kwargs[feature] = bounds[1]
        return feature_curve(
            classifier,
            feature,
            {"pfds": self.pfds, "target": self.target},
            bounds=bounds,
            Npts=Npts,
            pct=pct,
            plot=plot,
            ...
            for col in range(ncol):
                ...
                try:
                    ...
                    if feature in ["intervals", "subbands"]:
                        N = self.kwargs[feature]
                        ax.imshow(
                            self.test_data[sample_list[i]].reshape(N, N),
                            cmap=plt.cmap.gray,
                        )
                    else:
