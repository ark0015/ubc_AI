...
class MainFrameGTK(Gtk.Window):
    ...
    def __init__(self, data=None, tmpAI=None, spplot=False):
        ...
        if exists("%s/known_pulsars.pkl" % AI_path):
            self.knownpulsars = pickle.load(open("%s/known_pulsars.pkl" % AI_path))
        elif exists("known_pulsars.pkl"):
            self.knownpulsars = pickle.load(open("known_pulsars.pkl"))
        else:
        ...
        if self.data:
            self.voters = [
                name for name in self.data.dtype.names[1:] if not name.endswith("_FL")
            ]
            ...
            if len(self.voters) > 2:
                ...
                if "AI" in self.voters:
                    self.active_col2 = self.col_options.index(
                        self.voters[self.active_voter]
                    )
                    self.col2.set_active(self.active_col2)
                else:
                    self.active_col2 = self.col_options.index(
                        self.voters[self.active_voter + 1]
                    )
                    self.col2.set_active(self.active_col2)
    ...
    def on_view_limit_changed(self, widget):
        ...
        if idx1 != idx2:
            ...
            if data.ndim == 0:
                dtyp = [
                    (name, self.data.dtype[name].str) for name in ["fname", col1, col2]
                ]
            ...
            if limidx.size > 1 and np.any(limidx) and limtog:
                ...
                self.statusbar.push(
                    0,
                    "Showing %s/%s candidates above %s"
                    % (limidx.sum(), len(limidx), lim),
                )
            else:
                self.statusbar.push(
                    0, "No %s candidates > %s. Showing all" % (col1, lim)
                )
            ...
            if data.ndim == 0:
                dtyp = [(name, self.data.dtype[name].str) for name in ["fname", col1]]
                data = np.array([data], dtype=dtyp)
    ...
    def on_DM_limit_val_changed(self, widget):
        ...
        self.statusbar.push(
            0, "removed %s candidates with DM < %s." % (removecount, lim)
        )
    ...
    def on_pfdwin_key_press_event(self, widget, event):
        ...
        votes = {
            ...
            "5": 0.5,
            "m": 0.5,  # 50/50 pulsar/rfi
            "k": 2.0,  # known pulsar
            "h": 3.0,  # harmonic of known
            "0": 0.0,  # rfi (not a pulsar)
        }
        ...
        if self.data:
            if key in votes:
                ...
                if FL:
                    ...
                    if value in [0, 1]:
                        ...
                        o.set_active(value)
                        fname = self.pfdstore_set_value(
                            value, this_iter=this_iter, return_fname=True, FL=FL
                        )
                else:
                    fname = self.pfdstore_set_value(
                        value, this_iter=this_iter, return_fname=True
                    )
            ...
            if 0 and (cand_vote in awards) | (
                (cand_vote > max(awards)) & (cand_vote % 500 == 0)
            ):
                try:
                ...
                if level < len(awards):
                    ...
                    nl = 500
                note += (
                    "Only %s votes to the next level, and possibly a free coffee from Aaron and Weiwei.\n"
                    % nl
                )
    ...
    def add_candidate_to_knownpulsars(self, fname):
        ...
        if exists(fname) and fname.endswith(".pfd"):
            ...
            if float(pfd.decstr.split(":")[0]) > 0:
                ...
                sgn = ""
                name = "J%s%s%s" % (
                    "".join(pfd.rastr.split(":")[:2]),
                    sgn,
                    "".join(pfd.decstr.split(":")[:2]),
                )
            #            this_pulsar = known_pulsars.pulsar(fname, name, ra, dec, p0*1e-3, dm)
            this_pulsar = known_pulsars.pulsar(
                fname, name, ra, dec, p0, dm, catalog="local"
            )
    ...
    def on_col_changed(self, widget):
        ...
        """
        if self.data and self.active_col1 and self.active_col2:
    ...
    def on_pfdtree_select_row(self, widget, event=None):  # , data=None):
        ...
        if tmpiter:
            ...
            if (
                exists(fname)
                and fname.endswith(".pfd")
                and (self.tmpAI)
                and self.tmpAI_tog.get_active()
            ):
            ...
            elif (self.tmpAI) and self.tmpAI_tog.get_active():
                avgs = {
                    "phasebins": np.nan,
                    "subbands": np.nan,
                    "intervals": np.nan,
            ...
            if not self.aiview_tog.get_active():
                if fpng and exists(fpng):
                    ...
                    self.image_disp.set_text(
                        "displaying : %s %s" % (basename(fname), disp_apnd)
                    )
                else:
                ...
                if exists(fname) and fname.endswith(".pfd"):
                    ...
                    if fpng and exists(fpng):
                        self.image.set_from_file(fpng)
                        self.image_disp.set_text(
                            "displaying : %s %s" % (fname, disp_apnd)
                        )
                    else:
                        note = "Failed to generate png file %s" % fname
                        print(note)
    ...
    def generate_AIviewfile(self, fname):
        ...
        * If tmpAI, we use its' pca/nbins
        ...
        vals = [
            ("pprof_nbins", "pprof_pcacomp", "phasebins"),  # pulse profile
        ...
        for subplt, inp in enumerate(vals):
            ...
            if subplt == 0:
                ...
                ax.imshow(data.reshape(nbins, nbins), cmap=plt.cm.gray_r)
    ...
    def check_AIviewfile_match(self, fname):
        ...
        """
        vals = [
            ("pprof_nbins", "pprof_pcacomp"),  # pulse profile
        ...
        if tmpiter:
            ...
            if len(pathlist) > 0:
                ...
                if self.knownpulsars.has_key(fname):
                    ...
                    z = [
                        v.catalog for v in self.knownpulsars.values() if v.name == fname
                    ]
                    if len(z) == 1:
    ...
    def on_tmpAI_toggled(self, event):
        ...
        if self.tmpAI_tog.get_active() and (self.tmpAI is None):
            ...
            if response == Gtk.ResponseType.OK:
                ...
                try:
                    self.tmpAI = pickle.load(open(fname))
    ...
    def update_tmpAI_votemat(self, avgs):
        ...
        if "overall" in avgs:
            self.tmpAI_overall.set_text(
                "overall voting performance: %0.4f" % avgs["overall"]
            )
        else:
    ...
    def on_aiview_change(self, event):
        ...
        vals = [
            "pprof_nbins",
            "pprof_pcacomp",  # pulse profile
            "si_nbins",
            "si_pcacomp",  # frequency subintervals
            "pi_bins",
            "pi_pcacomp",  # pulse intervals
            "dm_bins",
            "dm_pcacomp",  # DM-vs-chi2
        ]
    ...
    def on_open(self, event=None, fin=None):
        ...
        if self.knownpulsars == None:
            ...
            self.knownpulsars = known_pulsars.get_allpulsars()
            self.statusbar.push(
                0, "Downloaded %s known pulsars for x-ref" % len(self.knownpulsars)
            )
    ...
    def find_matches(self):
        ...
        if tmpiter:
            ...
            if exists(fname) and (pfd or arch or spd):
                ...
                self.pmatch_store.append(
                    [
                        nm,
                        str(np.round(this_pulsar.P0, 5)),
                        this_pulsar.DM,
                        this_pulsar.ra,
                        this_pulsar.dec,
                        this_vote,
                ...
                for m in match_nonlocal_loc:
                    ...
                    if num == 0:
                        num = 1
                        den = max_denom
                    pdiff = abs(1.0 - float(den) / float(num) * this_pulsar.P0 / m.P0)
                    if verbose:
                        print(
                            "position match (name, ra, dec, DM): (%s, %s, %s, %s)"
                    ...
                    if (m.DM != np.nan) and (this_pulsar.DM != 0.0):
                        ...
                        if dDM > 0.15:
                            if verbose:
                                print(
                                    "  rejecting %s since Delta DM/DM = %s"
                                    % (m.name, dDM)
                                )
                            continue
    ...
    def on_update_knownpulsars(self, widget):
        ...
        if self.knownpulsars == None:
            ...
            self.statusbar.push(
                0, "Saved list of %s pulsars to %s" % (len(self.knownpulsars), fout)
            )
        else:
            if len(newlist) > len(self.knownpulsars):
                ...
                self.knownpulsars = newlist
                self.statusbar.push(
                    0, "Added %s new pulsars. Saving to %s" % (n_new, fout)
                )
                pickle.dump(self.knownpulsars, open(fout, "w"))
            else:
    ...
    def on_sampleqry_changed(self, event):
        ...
        """
        tmplts = {
            "by candidate id": "SELECT * FROM PDM_Candidate_Binaries_Filesystem as t where t.pdm_cand_id = %s",
    ...
    def on_qry_execute(self, event):
        ...
        # if "by candidateid" != active we still use that query to download the filelocation
        if (
            qry
            != "SELECT * FROM PDM_Candidate_Binaries_Filesystem as t where t.pdm_cand_id = %s"
        ):
        ...
        if (self.qrybasedir) and (not os.path.exists(self.qrybasedir)):
            os.makedirs(self.qrybasedir)
        temp = tempfile.NamedTemporaryFile(
            prefix="PFDqry_", suffix=".txt", dir=self.qrybasedir, delete=False
        )
        if self.active_voter is None:
            # prompt for voter name
            name = inputbox(
                "Voter chooser", "No user voters found in %s. Add your voting name"
            )
            if name:
                act_name = name
            else:
...
def convert(fin):
    ...
    if fin.endswith(".pfd"):
        ...
        if show_pfd:
            ...
            subprocess.call(cmd, shell=False, stdout=open("/dev/null", "w"))
...
def load_data(fname):
    ...
    *if a voter_FL column is present but not 'voter', we copy the overall
    ...
    # fin.close()
        ...
        if "#" not in l1:
            print(
                "[optional] first line expected to be a comment line describing the columns"
            )
        ...
        try:
            data = np.recfromtxt(
                fname, dtype={"names": colnames, "formats": coltypes}, comments="#"
            )
        except (IOError):
            data = None
    ...
    if data:
        ...
        if len(data.dtype.names) == 2:
            ...
            if name.endswith("_FL"):
                ...
                print(
                    "Found feature-label voter %s. Copying overall vote to regular column %s "
                    % (name, newname)
                )
...
def inputbox(title="Input Box", label="Please input the value", parent=None, text=""):
    """
...
def messagedialog(
    ...
    d = Gtk.MessageDialog(
        parent=parent, flags=Gtk.DialogFlags.MODAL, type=dialog_type, buttons=buttons
    )
...
def feature_predict(clf, pfd):
    ...
    for f in features:
        if clf.strategy != "adaboost":
            avgs[f] = np.mean(
                [
                    c.predict_proba(pfd)[..., 1][0]
                    for c in clf.list_of_AIs
                    if f in c.feature
                ]
            )
        else:
            ...
            avgs[f] = np.sum(
                [
                    (c.predict_proba(pfd)[..., 1][0] * 2.0 - 1) * weights[i]
                    for i, c in enumerate(clf.list_of_AIs)
                    if f in c.feature
                ]
            )
            ...
            avgs[f] = (avgs[f] + max_psr) / max_psr / 2.0
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--datafile",
        dest="data",
        help="load pfd list from file FILE",
        metavar="FILE",
    )
    parser.add_option(
        "-a",
        "--tmpAI",
        dest="tmpAI",
        help="load a temporary AI clf.pkl to gauge it's performance and see its' view of the candidates",
        metavar="clf.pkl",
    )
