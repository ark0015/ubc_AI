...
def plot_histogram(probs, target, title=False):
    ...
    npsr_clf, binpsr, patchespsr = plt.hist(
        probs[target == 1],
        25,
        facecolor="grey",
        alpha=0.65,
        label="pulsars (%s)" % npulsar,
        range=[0, 1],
    )

    nrfi_clf, binrfi_clf, patchesrfi_clf = plt.hist(
        probs[target != 1],
        25,
        facecolor="w",
        alpha=0.65,
...
def find_best_f1(proba, target):
    ...
    bestprec = 0.0
    bestcompl = 0.0
    for pct in np.linspace(0.0, proba.max(), nbins, endpoint=False):
...
def cut_performance(
    AIs, target, nbins=25, plot=True, norm=True, legend=True, features=None
):
    """
    ...
    for k, v in AIs.iteritems():
        ...
        if v.ndim == 1:
            idcs = label == 1
            psr_hist[k] = np.histogram(v[idcs], nbins, range=[0, 1])[
                0
            ]  # returns histogram, bin_edges
            idcs = label != 1
            rfi_hist[k] = np.histogram(v[idcs], nbins, range=[0, 1])[0]
        else:
            psr_hist[k] = np.histogram(v[label == 1][..., 1], bins=nbins, range=[0, 1])[
                0
            ]  # returns histogram, bin_edges
            rfi_hist[k] = np.histogram(v[label != 1][..., 1], bins=nbins, range=[0, 1])[
                0
            ]
...
def plot_neuralactivity(
    nn, data, cls=1, std=1, imshow=False, title="", savename=None, topN=None, shift=True
):
    """
...
def plot_combinedAI_shiftpredict(cAI, pfd):
    ...
    nbin = lopm.shape[0]
    coords = mgrid[0 : 1.0 - 1.0 / nbin : nbin * 1j]
    ...
    for fi, f in enumerate(cAI.list_of_AIs):
        ...
        if f.feature.keys()[0] != "DMbins":
            v = lop[fi]
            ax1.plot(coords, v, next(linecycler), color=next(colorcycler), label=lbl)
...
def plot_classifier_shiftpredict(clf, pfd, compare=None):
    ...
    preds = []
    x = mgrid[0 : 1.0 - 1.0 / nbin : nbin * 1j]
    ...
    for shift in range(nbin):
        ...
        if clf.use_pca:
            ...
            ax1.plot(
                ...
                preds,
                "b",
                label="%s" % str(type(clf)).split(".")[-1].strip(">").strip("'"),
            )
